...
namespace ExcelCRUDWebAPI.Services
{
    public class ExcelServices
    {
        private readonly string _excelFilePath;

        public ExcelServices(IConfiguration configuration)
        {
        ...
        public List<PersonalInfo> ReadExcelData()
        {
            ...
            using (var package = new ExcelPackage(new FileInfo(_excelFilePath)))
            {
                ...
                for (int row = 2; row <= rowCount; row++)
                {
                    ...
                    {
                        if (worksheet.Cells[row, 2].Value is double)
                        {
                            double excelDateSerial = (double)worksheet.Cells[row, 2].Value;
                            DateTime baseDate = new DateTime(1900, 1, 1);
                            dateOfBirth = baseDate.AddDays(excelDateSerial - 2);
                        }
                        else
                        {
                            double excelDateSerial;
                            if (double.TryParse(worksheet.Cells[row, 2].Value.ToString(), out excelDateSerial))
                            {
                                DateTime baseDate = new DateTime(1900, 1, 1);
                                dateOfBirth = baseDate.AddDays(excelDateSerial - 2);
                            }
                            ...
                            {
                                throw new InvalidCastException("The cell value could not be converted to a double.");
                            }
                        }
                    }
